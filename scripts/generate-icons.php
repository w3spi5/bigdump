...
<?php
/**
 * BigDump SVG Icon Sprite Generator
 *
 * Generates an SVG sprite file containing all icons used in the application.
 * Icons are from Font Awesome 6.5.1 (solid style).
 *
 * Usage: php scripts/generate-icons.php
 */

// Font Awesome 6.5.1 SVG paths (solid icons)
...
$icons = [
    ...
    'play' => [
        'viewBox' => '0 0 384 512',
        'path' => 'M73 39c-14.8-9.1-33.4-9.4-48.5-.9S0 62.6 0 80L0 432c0 17.4 9.4 33.4 24.5 41.9s33.7 8.1 48.5-.9L361 297c14.3-8.8 23-24.3 23-41s-8.7-32.2-23-41L73 39z'
    ],
    'trash' => [
        'viewBox' => '0 0 448 512',
    ...
    // Loading state (spinner uses Tailwind animate-spin)
    'spinner' => [
        'viewBox' => '0 0 512 512',
...
$svg .= '<!--' . PHP_EOL;
$svg .= '  BigDump SVG Icon Sprite' . PHP_EOL;
$svg .= '  Generated: ' . date('Y-m-d H:i:s') . PHP_EOL;
$svg .= '  Icons: ' . count($icons) . PHP_EOL;
$svg .= '  Source: Font Awesome 6.5.1 (Solid)' . PHP_EOL;
$svg .= '-->' . PHP_EOL;
$svg .= '<svg xmlns="http://www.w3.org/2000/svg" style="display: none;">' . PHP_EOL;

foreach ($icons as $name => $icon) {
    $svg .= '  <symbol id="' . $name . '" viewBox="' . $icon['viewBox'] . '">' . PHP_EOL;
    $svg .= '    <path d="' . $icon['path'] . '"/>' . PHP_EOL;
    $svg .= '  </symbol>' . PHP_EOL;
}
...
// Write to file
$outputPath = __DIR__ . '/../assets/icons.svg';
$bytesWritten = file_put_contents($outputPath, $svg);

if ($bytesWritten === false) {
    echo "Error: Failed to write SVG sprite to {$outputPath}\n";
    exit(1);
}

echo "Success: Generated SVG sprite with " . count($icons) . " icons\n";
echo "Output: {$outputPath} (" . number_format($bytesWritten) . " bytes)\n";
echo "Icons: " . implode(', ', array_keys($icons)) . "\n";
